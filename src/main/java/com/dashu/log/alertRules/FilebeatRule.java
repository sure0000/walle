...
public class FilebeatRule {
    ...
    public void isAliveRule(String cmd,String hostname){
        ...
        if (!isAlive){
            ...
            notify.sendMessage("filebeat","the filebeat of "+hostname+" is down!","filebeat-down");
            logger.warn("the filebeat of "+hostname+" is down!");
            RunCmdLine runCmdLine = new RunCmdLine(this);
            List ret = runCmdLine.callShell(cmd);
            if (ret == null){
                notify.sendMessage("filebeat","we try to up the filebeat of "+hostname+",but fail please check it!","filebeat-start-fail");
                logger.warn("we try to up the filebeat of "+hostname+",but fail please check it!");
            }else{
                notify.sendMessage("filebeat","the filebeat of "+hostname+" is up!","filebeat-start");
